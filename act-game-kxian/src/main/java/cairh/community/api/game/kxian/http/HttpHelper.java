package cairh.community.api.game.kxian.http;
...
public class HttpHelper {
	public static int DEFAULT_CONNECTION_TIMEOUT = 60;
	...
	public static HttpResult postJson(String url, String data, String encoding)
			throws Exception {
		return HttpHelper.postJson(url, data, encoding, 90, null, null, null, null);
	}
	...
	public static HttpResult postJson(String url, String data, String encoding,
		...
		return HttpHelper.postJson(url, postData, encoding, timeOut, cookies,
				headers, localAddress, proxy);
	}
	...
	public static HttpResult postJson(String url, byte[] data, String encoding,
			int timeOut, CookieStore cookies, List<Header> headers,
			String localAddress, HttpHost proxy) throws Exception {
	...
	public static HttpResult put(String url, String data, String encoding)
			throws Exception {
		return HttpHelper.put(url, data.getBytes(encoding), encoding, 90, null, null, null, null);
	}
	
	public static HttpResult put(String url, byte[] data, String encoding,
			int timeOut, CookieStore cookies, List<Header> headers,
			String localAddress, HttpHost proxy) throws Exception {
		...
		if (contentType != null) {
			httput.setHeader("Content-Type", contentType);
		} else {
			if (data != null && data.length > 0) {
				httput.setHeader("Content-Type",
						"application/json");
			}
		}
		
		try {
			httput.setEntity(new ByteArrayEntity(data));
		} catch (Exception e) {
			throw e;
		}
		...
		HttpResponse response;
		try {
			response = httpclient.execute(httput, localContext);
			httpResult = new HttpResult(localContext, response);
		} catch (Exception e) {
			httput.abort();
			throw e;
...
//			}
	...
	public static HttpResult post(String url, String requestInfoName, String requestInfo, String requestFileName, File file, String fileName, String encoding,
		...
		//file --> InputStream --> InputStreamBody
